...
```bash
./gradlew 04.3-open-telemetry:open-telemetry-consumer:build 04.3-open-telemetry:open-telemetry-provider:build
docker-compose -f 04.3-open-telemetry/docker-compose.yaml up --abort-on-container-exit
```

Once the consumer and provider are up, start a contract negotiation by executing:

```bash
curl -X POST -H "Content-Type: application/json" -H "X-Api-Key: password" -d @04.3-open-telemetry/contractoffer.json "http://localhost:9192/api/v1/management/contractnegotiations"
```
...
```bash
curl -X POST -H "Content-Type: application/json" -H "X-Api-Key: password" -d @04.3-open-telemetry/filetransfer.json "http://localhost:9192/api/v1/management/transferprocess"
```
...
```yaml
  consumer:
    ...
    entrypoint: java
      ...
      -jar /samples/04.3-open-telemetry/open-telemetry-consumer/build/libs/consumer.jar
```
